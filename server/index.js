...
// ── /api/state routes (MySQL key-value store) ────────────────────────────────
registerStateRoutes(app);

// ── /api/data/:key — alias kompatibilitas (pakai tabel app_state yang sama) ──
app.get('/api/data/:key', async (req, res) => {
  try {
    ...
    const value = rows.length ? JSON.parse(rows[0].state_value) : null;
